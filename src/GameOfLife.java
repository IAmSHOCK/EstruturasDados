...
class Game 
{
    ...
    {
	   int count = 0;
	   if(m[x+1][y] == ALIVE && (x+1)<rows)                 count++;
       if(m[x-1][y] == ALIVE && (x-1)>0)                    count++;
       if(m[x][y+1] == ALIVE && (y+1)>cols)                 count++;
       if(m[x][y-1] == ALIVE && (y-1)>0)                    count++;
       if(m[x+1][y+1] == ALIVE && (x+1)<rows && (y+1)>cols) count++;
       if(m[x-1][y+1] == ALIVE && (x-1)>0 && (y+1)>cols)    count++;
       if(m[x+1][y-1] == ALIVE && (x+1)<rows && (y-1)>0)    count++;
       if(m[x-1][y-1] == ALIVE && (x-1)>0 && (y-1)>0)       count++;
	   return count;
    }
